...
LASER_ACTIVE_DURATION = 300    # 5초 (60 FPS * 5 = 300 프레임)
MAX_ACTIVE_LASERS = 3
LASER_THICKNESS = 30        
...
def main():
    ...
    while True:
        ...
        # --- 레이저 그리기 파트 변경 (노란색/빨간색 점멸) ---
        for l in lasers:
            if l["state"] == "WARNING":
                # 남은 타이머 프레임을 이용해 색상을 번갈아 깜빡이게 만듭니다. (5프레임 주기)
                blink_color = RED if (int(l["timer"]) // 5) % 2 == 0 else YELLOW
                pygame.draw.rect(canvas, blink_color, l["rect"], 3)
                canvas.blit(font.render('!', True, blink_color), l["rect"].center)
            else: 
