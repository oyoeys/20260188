...
def main():
    ...
    while True:
        ...
        for laser in lasers:
            ...
            # --- 레이저가 벽과 닿는 부분에서 불꽃이 터지는 효과 ---
            if laser["state"] == "ACTIVE":
                r = laser["rect"]
                is_horizontal = r.width > r.height
                
                # 매 프레임마다 양쪽 벽에서 불꽃 파티클 생성 (개수 조절 가능)
                for _ in range(random.randint(3, 5)): 
                    if is_horizontal:
                        # 가로 레이저: 왼쪽 벽(x=0)과 오른쪽 벽(x=sw)에서 스폰
                        spawn_left = random.choice([True, False])
                        if spawn_left:
                            px = 0
                            py = random.uniform(r.top, r.bottom)
                            pvx = random.uniform(2, 6)   # 오른쪽(안쪽)으로 강하게 튀어오름
                            pvy = random.uniform(-2, 2)  # 위아래로 약간 퍼짐
                        else:
                            px = sw
                            py = random.uniform(r.top, r.bottom)
                            pvx = random.uniform(-6, -2) # 왼쪽(안쪽)으로 강하게 튀어오름
                            pvy = random.uniform(-2, 2)
                    else:
                        # 세로 레이저: 위쪽 벽(y=0)과 아래쪽 벽(y=sh)에서 스폰
                        spawn_top = random.choice([True, False])
                        if spawn_top:
                            py = 0
                            px = random.uniform(r.left, r.right)
                            pvy = random.uniform(2, 6)   # 아래쪽(안쪽)으로 강하게 튀어오름
                            pvx = random.uniform(-2, 2)
                        else:
                            py = sh
                            px = random.uniform(r.left, r.right)
                            pvy = random.uniform(-6, -2) # 위쪽(안쪽)으로 강하게 튀어오름
                            pvx = random.uniform(-2, 2)

                    particles.append({
                        "exact_x": px, "exact_y": py,
                        "speed_x": pvx, "speed_y": pvy,
                        "life": random.randint(15, 30), # 불꽃이 퍼지는 수명 조절
                        "start_life": 0 
                    })
        ...
        # --- 아주 짧게 튀는 큰 투명 점들(파티클) 그리기 ---
        # 파티클 전용 투명 서피스 생성
        particle_surf = pygame.Surface((sw, sh), pygame.SRCALPHA)
        particle_surf.fill((0, 0, 0, 0)) # 완전 투명으로 초기화

        for p in particles:
            life_ratio = p["life"] / p["start_life"]
            
            # 색상 결정 (투명도 포함, 기본 투명도 150)
            base_color = random.choice([RED, ORANGE])
            # life_ratio에 따라 서서히 사라지도록 설정할 수도 있음 (예: int(150 * life_ratio))
            color_with_alpha = (*base_color, 150) 
            
            # 크기 결정 (기존 4에서 12로 크게 늘림)
            size = max(1, int(12 * life_ratio)) 
            
            # 투명 서피스에 그리기
            pygame.draw.rect(particle_surf, color_with_alpha, (int(p["exact_x"]), int(p["exact_y"]), size, size))
