...
# --- 슬로우 게이지 설정 ---
MAX_SLOW_GAUGE = 180  # 135에서 180으로 증가
SLOW_CONSUME_RATE = 1
            ...
            if is_phase_mid:
                ...
                if laser_spawn_timer >= level_cfg["laser_interval"] and len(lasers) < MAX_ACTIVE_LASERS:
                    ...
                    if random.choice([True, False]):
                        ...
                        lasers.append({"state": "WARNING", "rect": pygame.Rect(-500, ly, 3000, LASER_THICKNESS), "timer": LASER_WARNING_DURATION, "duration": level_cfg["laser_duration"], "origin": random.choice(["left", "right"]), "image": laser_img})
                    else:
                        lx = random.randint(0, sw - LASER_THICKNESS)
                        lasers.append({"state": "WARNING", "rect": pygame.Rect(lx, -500, LASER_THICKNESS, 3000), "timer": LASER_WARNING_DURATION, "duration": level_cfg["laser_duration"], "origin": random.choice(["top", "bottom"]), "image": laser_img})
        ...
        for l in lasers:
            if l["state"] == "WARNING":
                ...
                if l.get("image"):
                    ...
                    if is_horizontal:
                        # 두께(세로)에 맞추고 가로 방향으로 타일처럼 이어 붙이기
                        tile_img = pygame.transform.scale(l["image"], (img_w, l["rect"].height))
                        for x in range(l["rect"].left, l["rect"].right, img_w):
                            canvas.blit(tile_img, (x, l["rect"].top))
                    else:
                        # 두께(가로)에 맞추고 세로 방향으로 타일처럼 이어 붙이기
                        tile_img = pygame.transform.scale(l["image"], (l["rect"].width, img_h))
                        for y in range(l["rect"].top, l["rect"].bottom, img_h):
