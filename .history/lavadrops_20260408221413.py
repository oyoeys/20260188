    ...
    while True:
            ...
            if is_phase_mid:
                ...
                if laser_spawn_timer >= level_cfg["laser_interval"] and len(lasers) < MAX_ACTIVE_LASERS:
                    ...
                    if random.choice([True, False]):
                        ...
                        # 생성 시점에 발사 방향(origin)을 고정 (왼쪽 벽 혹은 오른쪽 벽)
                        ...
                        # 생성 시점에 발사 방향(origin)을 고정 (위쪽 벽 혹은 아래쪽 벽)
        ...
        for laser in lasers:
            ...
            if laser["state"] == "ACTIVE":
                ...
                origin = laser.get("origin", "left") # 안전장치: 혹시 없다면 기본값 left
                
                # 매 프레임마다 지정된 벽에서 불꽃 파티클 생성 (개수를 약간 늘려 풍성하게)
                for _ in range(random.randint(4, 7)): 
                    if is_horizontal:
                        if origin == "left":
                            px = 0
                            py = random.uniform(r.top, r.bottom)
                            pvx = random.uniform(3, 8)       # 안쪽(오른쪽)으로 뿜어짐
                            pvy = random.uniform(-2.5, 2.5)  # 위아래로 살짝 퍼짐
                        else: # origin == "right"
                            px = sw
                            py = random.uniform(r.top, r.bottom)
                            pvx = random.uniform(-8, -3)     # 안쪽(왼쪽)으로 뿜어짐
                            pvy = random.uniform(-2.5, 2.5)
                    else:
                        if origin == "top":
                            py = 0
                            px = random.uniform(r.left, r.right)
                            pvy = random.uniform(3, 8)       # 안쪽(아래)으로 뿜어짐
                            pvx = random.uniform(-2.5, 2.5)
                        else: # origin == "bottom"
                            py = sh
                            px = random.uniform(r.left, r.right)
                            pvy = random.uniform(-8, -3)     # 안쪽(위)으로 뿜어짐
                            pvx = random.uniform(-2.5, 2.5)
        ...
        for p in particles:
            ...
            color_with_alpha = (*base_color, 150) # 투명도 고정 적용
