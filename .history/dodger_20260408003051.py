...
def draw_hud(display_score, level_cfg, lives):
    screen.blit(font.render(f"Score: {display_score}", True, WHITE), (10, 10))
...
def main():
    ...
    enemies = []
    score = 0.0 # 내부 점수는 소수점으로 정밀하게 계산
    lives = 3
    level_idx = 0
    ...
    while True:
        ...
        display_score = int(score) # 화면에 보여줄 정수 점수
        ...
        for enemy in enemies:
            ...
            if not is_out_of_bounds:
                survived.append(enemy) # 안 나갔으면 리스트에 보관
            else:
                score += enemy["score_value"] # 나갔으면 점수만 주고 증발
                
        enemies = survived # 핵심: 살아남은 적들로 리스트 덮어씌우기 (완벽 삭제)
        ...
        draw_hud(display_score, level_cfg, lives)
        pygame.display.flip()
