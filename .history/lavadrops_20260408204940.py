...
RED    = (220, 50,  50)
YELLOW = (240, 200, 0)
ORANGE = (255, 165, 0) # 파티클 효과용 주황색 추가
GRAY   = (40,  40,  40)
CYAN   = (0,   255, 255) 
GREEN  = (0,   255, 0)
...
def draw_hud(surface, display_score, level_cfg, lives, current_phase):
    ...
    elif current_phase == 3: phase_text = " [FINAL PHASE]" # LAST PHASE에서 FINAL PHASE로 변경됨
    elif current_phase == 2: phase_text = " [LAST PHASE]"  # LATE PHASE에서 LAST PHASE로 변경됨
...
def main():
    ...
    while True:
        ...
        if current_phase > last_phase:
            ...
            enemies.clear() 
            lasers.clear()  
            particles.clear() # 페이즈 변경 시 파티클도 초기화
            shake_timer = 60 
            shake_magnitude = 30 
        ...
        if shake_timer <= 0:
            ...
            if v_spawn_timer >= target_v_spawn:
                ...
                enemies.append({"rect": pygame.Rect(x, -ENEMY_H, ENEMY_W, ENEMY_H), "exact_x": float(x), "exact_y": float(-ENEMY_H), "speed_x": 0, "speed_y": speed, "score_value": 1.0 / width_ratio})
            ...
            if is_phase_mid:
                ...
                if laser_spawn_timer >= level_cfg["laser_interval"] and len(lasers) < MAX_ACTIVE_LASERS:
                    ...
                    if random.choice([True, False]):
                        ...
                        lasers.append({"state": "WARNING", "rect": pygame.Rect(-500, ly, 3000, LASER_THICKNESS), "timer": LASER_WARNING_DURATION, "duration": level_cfg["laser_duration"]})
                    else:
                        lx = random.randint(0, sw - LASER_THICKNESS)
                        lasers.append({"state": "WARNING", "rect": pygame.Rect(lx, -500, LASER_THICKNESS, 3000), "timer": LASER_WARNING_DURATION, "duration": level_cfg["laser_duration"]})
        ...
        for laser in lasers:
            ...
            # --- 레이저가 발사 중일 때 테두리에서 점들이 튀어나오는 효과 추가 ---
            if laser["state"] == "ACTIVE":
                r = laser["rect"]
                # FPS마다 2~3개의 점을 랜덤하게 생성
                for _ in range(random.randint(2, 3)):
                    is_horizontal = r.width > r.height
                    if is_horizontal:
                        px = random.uniform(r.left, r.right)
                        py = random.choice([r.top, r.bottom]) # 위나 아래 테두리 중 선택
                        pvx = random.uniform(-1, 1) # 좌우로 약간 퍼짐
                        # 테두리 방향에 따라 위나 아래로 솟구침
                        pvy = random.uniform(-4, -1) if py == r.top else random.uniform(1, 4)
                    else:
                        py = random.uniform(r.top, r.bottom)
                        px = random.choice([r.left, r.right]) # 왼쪽이나 오른쪽 테두리 선택
                        pvy = random.uniform(-1, 1) # 위아래로 약간 퍼짐
                        # 테두리 방향에 따라 좌나 우로 솟구침
                        pvx = random.uniform(-4, -1) if px == r.left else random.uniform(1, 4)

                    particles.append({
                        "exact_x": px, "exact_y": py,
                        "speed_x": pvx, "speed_y": pvy,
                        "life": random.randint(10, 20), # 아주 짧은 수명 (프레임)
                        "start_life": 0 # 초기 수명 저장용 (나중에 크기 조절에 사용)
                    })
        ...
        if invincible > 0: invincible -= 1
            ...
            if collision:
                ...
                enemies.clear(); lasers.clear(); particles.clear() # 충돌 시 파티클도 제거
        ...
        for p in particles:
            ...
            pygame.draw.rect(canvas, color, (int(p["exact_x"]), int(p["exact_y"]), size, size))
        ...
        for e in enemies: pygame.draw.rect(canvas, RED, e["rect"])
        if is_slow_active:
