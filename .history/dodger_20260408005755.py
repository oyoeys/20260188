    ...
    while True:
        ...
        for e in pygame.event.get():
            ...
            if e.type == pygame.VIDEORESIZE:
                ...
                return main_menu() # 창 크기 변하면 다시 그리기
            if e.type == pygame.KEYDOWN:
                if e.key == pygame.K_SPACE:
                    return True # 스페이스 누르면 게임 시작 신호 보냄
                if e.key == pygame.K_q:
                    return False # Q 누르면 종료
...
# 실행 흐름 제어 (메뉴 -> 게임 -> 게임오버 -> 반복)
if __name__ == "__main__":
    while True:
        # 1. 메인 메뉴 실행
        start_game = main_menu()
        if not start_game: # 메인 메뉴에서 Q를 누르거나 창을 닫으면 종료
            break
            
        # 2. 게임 실행
        restart = main()
        if not restart:  # 게임오버 창에서 Q를 누르거나 창을 닫으면 종료
            break
