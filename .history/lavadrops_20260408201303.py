...
LASER_ACTIVE_DURATION = 40     # 기존 20에서 40으로 2배 추가 증가
MAX_ACTIVE_LASERS = 3
LASER_THICKNESS = 30        
...
def draw_hud(surface, display_score, level_cfg, lives, current_phase):
    ...
    elif current_phase == 3: phase_text = " [LAST PHASE]"
    elif current_phase == 2: phase_text = " [LATE PHASE]"
    ...
    while True:
            ...
            if is_phase_mid:
                ...
                if laser_spawn_timer >= LASER_SPAWN_INTERVAL and len(lasers) < MAX_ACTIVE_LASERS:
                    ...
                    if random.choice([True, False]):
                        ...
                        lasers.append({"state": "WARNING", "rect": pygame.Rect(-500, ly, 3000, LASER_THICKNESS), "timer": LASER_WARNING_DURATION})
                    else:
                        lx = random.randint(0, sw - LASER_THICKNESS)
                        lasers.append({"state": "WARNING", "rect": pygame.Rect(lx, -500, LASER_THICKNESS, 3000), "timer": LASER_WARNING_DURATION})
        ...
        for laser in lasers:
            ...
            if laser["state"] == "WARNING" and laser["timer"] <= 0:
                laser["state"] = "ACTIVE"; laser["timer"] = LASER_ACTIVE_DURATION
            if laser["timer"] > 0: updated_lasers.append(laser)
        ...
        for l in lasers:
            if l["state"] == "WARNING":
                pygame.draw.rect(canvas, RED, l["rect"], 3)
                canvas.blit(font.render('!', True, RED), l["rect"].center)
