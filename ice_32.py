...
pygame.display.set_caption("Lava Drops - Dual BGM Edition")
...
LEVELS = [
    ...
    {"min_speed": 4, "max_speed": 7,  "spawn": 15, "label": "Lv.3",   "laser_interval": 120, "laser_duration": 180},
    {"min_speed": 5, "max_speed": 7,  "spawn": 10, "label": "Lv.4",   "laser_interval": 90,  "laser_duration": 120},
    {"min_speed": 5, "max_speed": 8,  "spawn": 8,  "label": "Lv.Max", "laser_interval": 60,  "laser_duration": 60},
]
...
def main():
    ...
    # --- [인게임 BGM] 게임 시작 시 PlayBGM.ogg로 교체 ---
    try:
        pygame.mixer.music.load("PlayBGM.ogg")
        pygame.mixer.music.set_volume(0.5)
        pygame.mixer.music.play(-1)
    except Exception as e:
    ...
    while True:
        ...
        if shake_timer <= 0:
            ...
            if is_phase_mid:
                ...
                if laser_spawn_timer >= level_cfg["laser_interval"] and len(lasers) < MAX_ACTIVE_LASERS:
                    laser_spawn_timer = 0
                    laser_img = random.choice(lava_images) if lava_images else None
                    if random.choice([True, False]):
        ...
        for laser in lasers:
            ...
            if laser["state"] == "ACTIVE":
                ...
                if origin in ("left", "top"):
                    laser["offset"] = laser.get("offset", 0.0) + flow_speed
                else:
                    laser["offset"] = laser.get("offset", 0.0) - flow_speed
        ...
        survived = []
        for e in enemies:
            if -ENEMY_W <= e["rect"].x <= sw and -ENEMY_H <= e["rect"].y <= sh: 
                survived.append(e)
            else: 
                score += e["score_value"]
        enemies = survived
        ...
        for l in lasers:
            if l["state"] == "WARNING":
                ...
                if l.get("image"):
                    is_horizontal = l["rect"].width > l["rect"].height
                    img_w, img_h = l["image"].get_size()
                    offset = int(l.get("offset", 0))
                    
                    if is_horizontal:
                        tile_img = pygame.transform.scale(l["image"], (img_w, l["rect"].height))
                        start_x = l["rect"].left + (offset % img_w) - img_w
                        for x in range(start_x, l["rect"].right, img_w):
                            canvas.blit(tile_img, (x, l["rect"].top))
                    else:
                        tile_img = pygame.transform.scale(l["image"], (l["rect"].width, img_h))
                        start_y = l["rect"].top + (offset % img_h) - img_h
                        for y in range(start_y, l["rect"].bottom, img_h):
                            canvas.blit(tile_img, (l["rect"].left, y))
                else:
        ...
        for p in particles:
            ...
            offset_p = size // 2
            pygame.draw.rect(particle_surf, color_with_alpha, (int(p["exact_x"]) - offset_p, int(p["exact_y"]) - offset_p, size, size))
